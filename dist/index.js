...
var CronToolkit = class {
  ...
  _timeZone = "UTC";
  _utcOffset = 0;
  _beginEpoch = import_luxon.DateTime.now().minus({ years: 10 }).toSeconds();
  _endEpoch = import_luxon.DateTime.now().plus({ years: 10 }).toSeconds();
  get timeZone() {
    return this._timeZone;
  }
  set timeZone(tz) {
    const dt = import_luxon.DateTime.now().setZone(tz);
  ...
  next(from) {
    const baseSeconds = from ?? import_luxon.DateTime.now().toSeconds();
    let dt = import_luxon.DateTime.fromSeconds(baseSeconds + this._utcOffset * 60, { zone: "UTC" }).plus({ seconds: 1 });
  ...
  previous(from) {
    const baseSeconds = from ?? import_luxon.DateTime.now().toSeconds();
    let dt = import_luxon.DateTime.fromSeconds(baseSeconds + this._utcOffset * 60, { zone: "UTC" }).minus({ seconds: 1 });
    NODE: for (let i = 0; i <= 2; i++) {
      const node = this.nodes[i];
      const field = node.fieldType;
      const curval = field === "second" ? dt.second : field === "minute" ? dt.minute : dt.hour;
