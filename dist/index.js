...
function uncomputeOrdinal(year, ordinal3) {
  const table = isLeapYear(year) ? leapLadder : nonLeapLadder, month0 = table.findIndex((i) => i < ordinal3), day = ordinal3 - table[month0];
...
function gregorianToWeek(gregObj, minDaysInFirstWeek = 4, startOfWeek = 1) {
  const { year, month, day } = gregObj, ordinal3 = computeOrdinal(year, month, day), weekday = isoWeekdayToLocal(dayOfWeek(year, month, day), startOfWeek);
  let weekNumber = Math.floor((ordinal3 - weekday + 14 - minDaysInFirstWeek) / 7), weekYear;
  if (weekNumber < 1) {
    weekYear = year - 1;
...
function weekToGregorian(weekData, minDaysInFirstWeek = 4, startOfWeek = 1) {
  ...
  let ordinal3 = weekNumber * 7 + weekday - weekdayOfJan4 - 7 + minDaysInFirstWeek, year;
  if (ordinal3 < 1) {
    year = weekYear - 1;
    ordinal3 += daysInYear(year);
  } else if (ordinal3 > yearInDays) {
    year = weekYear + 1;
    ordinal3 -= daysInYear(weekYear);
  } else {
    year = weekYear;
  }
  const { month, day } = uncomputeOrdinal(year, ordinal3);
  return { year, month, day, ...timeObject(weekData) };
}
function gregorianToOrdinal(gregData) {
  const { year, month, day } = gregData;
  const ordinal3 = computeOrdinal(year, month, day);
  return { year, ordinal: ordinal3, ...timeObject(gregData) };
}
function ordinalToGregorian(ordinalData) {
  const { year, ordinal: ordinal3 } = ordinalData;
  const { month, day } = uncomputeOrdinal(year, ordinal3);
...
var StepPattern = class extends CronPattern {
  ...
  toEnglish() {
    let s2 = `every ${this.step} ${this.englishUnit()}`;
    if (this.step != 1) {
      s2 += "s";
    }
    ;
...
var LastPattern = class extends CronPattern {
  offset = 0;
  dow;
  constructor(fieldType, offset2 = 0, dow) {
    super(fieldType);
    this.offset = offset2;
    this.dow = dow;
  }
...
var CronToolkit = class {
  ...
  buildNode(field, value) {
    ...
    if (/^\d+$/.test(value)) {
      let n2 = +value;
      if (field === "dow" && n2 === 0) n2 = 7;
      if (n2 < min || n2 > max) throw new Error(`${field} ${n2} out of range [${min}-${max}]`);
      return new SinglePattern(field, n2);
    }
    if (value.includes(",")) {
    ...
    if (/^(\*|\d+)-(\d+)$/.test(value)) {
      ...
      let start = match2[1] === "*" ? min : +match2[1];
      let end = +match2[2];
    ...
    if (field === "dom") {
      ...
      if (/^L-?(\d*)$/.test(value)) {
        const offset2 = value === "L" ? 0 : +(value.match(/L-?(\d+)/)?.[1] || 0);
        return new LastPattern(field, offset2);
      }
    }
    if (field === "dow") {
  ...
  describe() {
    ...
    const singleHMS = hmsNodes.every((n2) => n2 instanceof SinglePattern);
    let hms = "";
    if (wildHMS && hmsNodes.length > 0) {
      hms = hmsNodes[0].toEnglish();
    } else if (singleHMS && hmsNodes.length === 3) {
      const [s2, m, h] = hmsNodes.map((n2) => n2.value);
      hms = formatTime(s2, m, h);
    } else {
      hms = filtered.slice(0, 3).filter(
        (n2) => n2 !== null && !(n2 instanceof SinglePattern && n2.value === 0)
      ).map((n2) => n2.toEnglish()).join(" of ");
    }
    let rest = "";
  ...
  asString() {
    return this.nodes.map((n2) => n2.toString()).join(" ");
  }
  asQuartzString() {
